...
Fix: declare the dependency in the workspace that imports it, then run pnpm install followed by pnpm install --frozen-lockfile.

Verification: git clone the repo to a scratch directory, pnpm install --frozen-lockfile, and run pnpm run check there. Only a fresh clone reproduces CI's resolution; re-running checks in the working checkout proves nothing after dependency or import changes.
