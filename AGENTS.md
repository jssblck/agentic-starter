...
Repository skills live in `.agents/skills`; `.claude/skills` holds symlinks into it. Add a skill in one place and link it, and list both paths in `.gitignore`'s exception block. Shared skills come from `jssblck/agents`: `pnpm install` installs the repository's current HEAD (postinstall), and `pnpm run skills:update` refreshes an existing checkout. `skills-lock.json` is a local record, not a pin, and is ignored. The postinstall does not fail the install when the fetch fails; run `pnpm run skills:update` if skills are missing. Do not edit an installed skill in place; change it in `jssblck/agents`.
