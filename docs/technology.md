...
`.env` files broke in two places at once: a linked worktree does not inherit them, and a cloud sandbox never had them. A hosted secrets manager fixes the sharing and adds a session prompt per shell, a boot-time dependency, and a second system that has to agree with the code at deploy time; the sync step between that system and production is a recurring source of outages. sops with age puts the encrypted values in git. Every checkout, worktree, and sandbox has them after `git clone`; the only input anywhere is one age private key in a file or a variable; and a secret change is a commit that deploys with the code that needs it. The base separates access by identity instead of by environment variable plumbing: an `agent` key on every machine agents use decrypts dev, a `prod` key on the platform decrypts prod, and a personal key held in a password manager can be dropped into one checkout to elevate it. The personal key still lives in a password manager, which is now read once per elevation instead of once per command.
