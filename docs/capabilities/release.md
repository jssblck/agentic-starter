...
- `Dockerfile` for a Node process (Hono server, worker): a builder stage on `node:<major>-alpine` that runs `corepack enable`, copies `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `package.json`, `.npmrc`, `apps`, and `libs`, then `pnpm install --frozen-lockfile --prod --filter @scope/<app>...` (the app and every workspace package it depends on, in place) and `write-version`; a runtime stage on the same image that copies the whole `/repo`, runs `node apps/<app>/src/main.ts` as `node`, exposes the port, and has a `HEALTHCHECK` on `/api/health`. Do not use `pnpm deploy`: it moves workspace packages under `node_modules/.pnpm`, and Node refuses to strip types inside `node_modules` (`ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`), so `.ts` libs must stay outside it. Expect about 140 MB of `node_modules` for a worker with pg-boss, Drizzle, and OpenTelemetry.
