...
- `libs/version/src/generated.ts`: committed as `0.0.0` / `null` / `unknown`. `tools/write-version.ts --version X` rewrites it, every `package.json` version, the workspace entries in `bun.lock`, `[workspace.package].version` in `Cargo.toml`, and `build/version.json`. Never commit these rewrites from an ordinary branch.
...
- `AGENTS.md` check classification: "Release/tooling: run `bun run version:check` and `bun run release:check`."
...
- `.github/workflows/release.yml` on `push: tags: ['v*.*.*']`: `prepare` (validate tag, emit version), `binaries` matrix (checkout with `fetch-depth: 0`, setup Bun, toolchain if Rust, `bun install --frozen-lockfile`, `write-version`, `build-cli`, smoke test `--version`, package with README/LICENSE/NOTICE, upload artifact), `github-release` (download artifacts, add installers, `sha256sum <name>-* > checksums.txt`, `gh release create "$GITHUB_REF_NAME" artifacts/* --verify-tag --generate-notes`).
