...
Each page is a file under `src/routes`; the route tree regenerates during dev and build. The committed `routeTree.gen.ts` contains suppressions TanStack emits by design, so it is excluded from oxlint, oxfmt, and `nudge check` (the last through the root `.ignore`). Edit routes, never the generated file. Feature code lives beside its route in a feature directory (`src/todos`), with pure view logic in plain modules that test without a DOM.
